...
uint kmalloc_real(uint sz, int align, uint *phys)
{
    ...
    else //memory management is enabled
    {
		...
		if(phys != 0)
		{
			page_t *page = vmm_get_page((uint)addr, PDE_PRESENT | PDE_RW | PDE_USER, current_pde);
			*phys = ((*page & PDE_MASK) * 0x1000 + ((uint) addr & 0xFFF));
		}
		return (uint)addr;
    }
}
...
{		
    ...
    blocks = (uint*)kmalloc(n_blocks * sizeof(uint)); //deadlock..
...
void list_blocks()
{
    ...
    for(i = 0; i < nblocks; i++)
    {
        ...
        if(blocks[i] & FLAG_BLOCK_USED)
            cprint("0x%x [used  ] [0x%x]\n", GET_BLOCK_LOCATION(blocks[i]), GetBlockSize(i));
    }
    cprint("did not print %i free blocks ( 0x%x bytes of free memory .. )\n", (free_blocks - l), (free_blocks - l) * KMEM_CHUNK_SIZE);
}
void check_blocks()
{
	int i, j, e1, e2;
	cprint("Checking memory blocks.\n");
	for(i = 0; i < nblocks; i++)
	{
		if(blocks[i] & FLAG_BLOCK_USED)
		{
			...
			for(j = 0; j < nblocks; j++)
			{
				...
				if(GET_BLOCK_LOCATION(blocks[j]) > GET_BLOCK_LOCATION(blocks[i]) && GET_BLOCK_LOCATION(blocks[j]) < e1)
					cprint("Memory block %i collides with %i\n", i, j);
...
void *ralloc(uint sz, int align)
{
    ...
    if(idx == (uint)0xFFFFFFFF && sz >= KMEM_CHUNK_SIZE)
    {
        ...
        //cprint("Needs linkage (0x%x bytes)\n", sz);
        ...
        {
            //Panic ..
            //cprint("Could not link %x bytes of memory, no more memory available .. :(\n");
